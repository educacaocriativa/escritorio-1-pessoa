...
- **[CORRIGIDO — MNT-001, issue #311, 2026-09-05]** `audit.record(target='')` em **17 call sites**: `acc.id` ainda era `None` quando o `audit.record` rodava logo após o `db.add()`, e `None` chega à coluna (`String(255)`, `NOT NULL`, `default=""`) como `''` — **sem erro nenhum**. Por isso durou: a API respondia 201, a suíte ficava verde, e a trilha guardava uma linha que dizia que algo foi criado sem dizer o quê. Todos os 17 receberam `db.flush()` antes do `record` (em `chart_of_accounts`, `cost_centers`, `crm`, `products` e `whatsapp_templates` o flush entrou **dentro do `try`**, porque passa a ser ele, e não o commit, a levantar o `IntegrityError` do nome duplicado — o 409 tem teste). **A lição é a segunda metade:** o defeito estava documentado por nome e com contagem em `dna/eventos.py`, `bank/service.py`, `bank/origin.py`, `bank/transfers.py` e `core/facts.py` desde 2026-07-30, e continuou existindo por seis semanas — **documentar não conteve**. Contenção é `apps/api/tests/test_audit_target_flush_gate.py`, que reprova por AST qualquer `record(..., target=X.id)` alcançável depois de um `db.add(X)` sem flush, e prova contra fontes sintéticas que ele morde.
