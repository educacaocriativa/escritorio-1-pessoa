...
| 2026-08-18 | 1.0.1 | **Errata (issue #131), sem mudança de comportamento.** A premissa "o repo é da org `educacaocriativa`" usada para justificar o CLI em vez da `gitleaks-action` é falsa: `.owner.type` = `"User"` (conta pessoal), então o `GITLEAKS_LICENSE` nunca seria exigido aqui. A decisão **permanece** apoiada no argumento que se sustenta (Action sob EULA proprietário desde a v2.0.0 × CLI MIT, mesma engine). Corrigidos os comentários vivos de `ci.yml` e `.gitleaks.toml`; texto histórico preservado + errata na seção do Architect Review. Nenhum passo executável do workflow alterado (provado por `yaml.safe_load` antes/depois). | @dev |
...
> **Errata (2026-08-18 — issue #131), registrada sem reescrever o histórico abaixo.** A decisão 1
> (CLI em vez de Action) **continua correta**, mas uma das premissas usadas para justificá-la aqui,
> nas Tasks e nas Decisões do @devops é **falsa**: `educacaocriativa` **não é uma organização**, é
> uma **conta de usuário** (`gh api repos/educacaocriativa/escritorio-1-pessoa --jq .owner.type` →
> `"User"`). Logo o `GITLEAKS_LICENSE` **nunca seria exigido** neste repo — o EULA da Action pede
> chave apenas para repos de *Organization Account*; para *Personal Account* não há chave nem
> formulário de registro. Cai também, portanto, a "fricção de registro" citada abaixo. **O que
> sustenta a decisão é só a licença:** desde a v2.0.0 a `gitleaks-action` deixou de ser MIT e
> passou a um **EULA proprietário da Gitleaks LLC** (source-available, vincula pelo uso, proíbe
> modificar/derivar), enquanto o CLI/binário segue **MIT** com a mesma engine e cobertura
> idêntica. Os comentários vivos de `ci.yml` e `.gitleaks.toml` foram corrigidos; o texto
> histórico desta story fica como registro do que se acreditava em 2026-07-11.
